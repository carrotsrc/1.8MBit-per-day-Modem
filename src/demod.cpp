...
void resolve_state(magnitudes& mag, payload& pl) {
  ...
  if(mag.f0a > 100 | mag.f0b > 100) {
    handle_zero(mag, pl);
  } else if(mag.f1a > 100 || mag.f1b > 100) {
    handle_one(mag, pl);
  } else if(mag.fc > 400) {
    pl.ctl_marker = 'c';
    if(pl.bit_num > 0) {
      ...
      std::cout << "\n" << s.str() << "\n";
      std::cout << "\n\n" << decode_bin(pl.data_buffer, pl.bit_num) << std::endl;
    }
    pl.bit_num = 0;
  } 
}
...
int process(jack_nframes_t nframes, void* data) {
  ...
  mag.f0a = f0a < 0.0 ? f0a * -1.0 : f0a;
  mag.f0b = f0b < 0.0 ? f0b * -1.0 : f0b;
  mag.f1a = f1a < 0.0 ? f1a * -1.0 : f1a;
  mag.f1b = f1b < 0.0 ? f1b * -1.0 : f1b;
  mag.fc  = fc < 0.0 ? fc * -1.0 : fc;
  
  resolve_state(mag, *pl);

  auto print_fs = [&](float f0, float f1, float f2) {
...
int main(int argc, char** argv) {
    ...
    pl.taps = bpf(NUM_TAPS,44.1,0.2,0.9);
    pl.sample_buffer = create_buffer(NUM_TAPS);
    
    for(auto i =0; i < 5; i++){ std::cout << pl.fftw.bins[i] << " "; }
    std::cout << std::endl;
